        ...
        <div class="nav-tabs-custom">
            ...
            <div class="tab-content">
                <div class="tab-pane active">
                    <form method="GET" action="{{ route('admin.payment.reseller-payments.index') }}"
                        class="form-inline">
                            ...
                            <div class="row">
                                    ...
                                    <select class="form-control" name="reseller">
                                        ...
                                        @foreach ($resellers as $item)
                                            <option value="{{ $item->id }}"
                                                {{ Request::get('reseller') == $item->id ? 'selected' : '' }}>
                                                {{ $item->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <a class="btn btn-custom btn-flat"
                                        href="{{ route('admin.payment.reseller-payments.index') }}">X</a>
                                </div>
                            </div>
                        </div>
                    </form>
                                ...
                                <tbody>
                                    @foreach ($records as $val)
                                        <tr>
                                            ...
                                            <td>{{ $val->type }}</td>
                                            <td>{{ $val->reseller != null ? $val->reseller->name : '' }}</td>
                                            ...
                                            <td>
                                                <div class="dropdown">
                                                    ...
                                                    <ul class="dropdown-menu dropdown-menu-right">
                                                        @can('show reseller-payment')
                                                            <li><a
                                                                    href="{{ route('admin.payment.reseller-payments.show', $val->id) . qString() }}"><i
                                                                        class="fa fa-eye"></i> Show</a></li>
                                                        @endcan

                                                        @can('edit reseller-payment')
                                                            @if (!$val->approved_by && auth()->user()->id == $val->created_by)   
                                                                <li><a
                                                                        href="{{ route('admin.payment.reseller-payments.edit', $val->id) . qString() }}"><i
                                                                            class="fa fa-edit"></i> Edit</a></li>
                                                            @endif
                                                            @endcan
                                                            
                                                            @can('delete reseller-payment')
                                                                @if (!$val->approved_by && auth()->user()->id == $val->created_by)   
                                                                <li><a
                                                                    onclick="deleted('{{ route('admin.payment.reseller-payments.destroy', $val->id) . qString() }}')"><i
                                                                    class="fa fa-close"></i> Delete</a></li>
                                                                @endif
                                                        @endcan
                                                        @can('approval reseller-payment')
                                                            @if (auth()->user()->hasRole('Super Admin') || auth()->user()->hasRole('Branch Admin') || $val->created_by != auth()->user()->id)
                                                            @if(!$val->approved_by)
                                                            <li><a onclick="activity('{{ route('admin.payment.reseller-payments.approve', $val->id) . qString() }}', 'Are you sure to approve this payment?')"><i class="fa fa-pencil"></i> Approve</a></li>
                                                            @endif
                                                            @endif
                                                        @endcan
                                                    </ul>
