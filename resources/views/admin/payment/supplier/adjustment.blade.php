        ...
        <div class="nav-tabs-custom">
            ...
            <div class="tab-content">
                <div class="tab-pane active">
                    <div class="box-body">
                        <form method="POST"
                            action="{{ isset($data) ? route('admin.payment.supplier-payments.update', $data->id) : route('admin.payment.supplier-payments.store') }}{{ qString() }}"
                            ...
                            <div class="row">
                                <div class="col-sm-12">
                                    ...
                                    <div class="form-group{{ $errors->has('supplier_id') ? ' has-error' : '' }}">
                                        ...
                                        <div class="col-sm-9">
                                            <select name="supplier_id" class="form-control select2" id="adjust_supplier_id" required onchange="getAdjustDue()">
                                                <option value="">Select Supplier</option>
                                                @php($supplier_id = old('supplier_id', isset($data) ? $data->supplier_id : ''))
                                                @foreach ($suppliers as $cat)
                                                    <option value="{{ $cat->id }}"
                                                        {{ $supplier_id == $cat->id ? 'selected' : '' }}>
                                                        {{ $cat->name }}</option>
                                                @endforeach
                                            </select>
    ...
    <script>
        function getAdjustDue(){
            ...
            $.ajax({
                ...
                url: "{{ route('admin.user.suppliers.due') }}",
                beforeSend: () => {
                    $('#adjust_due').val('');
                },
                success: (res) => {
