        ...
        <div class="nav-tabs-custom">
            ...
            <div class="tab-content">
                <div class="tab-pane active">
                    <form method="GET" action="{{ route('admin.payment.supplier-payments.index') }}"
                        class="form-inline">
                            ...
                            <div class="row">
                                ...
                                <div class="form-group">
                                    ...
                                    <a class="btn btn-custom btn-flat"
                                        href="{{ route('admin.payment.supplier-payments.index') }}">X</a>
                                </div>
                            </div>
                        </div>
                    </form>
                                            ...
                                            <td>
                                                <div class="dropdown">
                                                    ...
                                                    <ul class="dropdown-menu dropdown-menu-right">
                                                        @can('show supplier-payment')
                                                            <li><a
                                                                    href="{{ route('admin.payment.supplier-payments.show', $val->id) . qString() }}"><i
                                                        ...
                                                        @can('edit supplier-payment')
                                                            @if (!$val->approved_by && auth()->user()->id == $val->created_by)   
                                                                <li><a
                                                                        href="{{ route('admin.payment.supplier-payments.edit', $val->id) . qString() }}"><i
                                                            ...
                                                            @can('delete supplier-payment')
                                                                @if (!$val->approved_by && auth()->user()->id == $val->created_by)   
                                                                <li><a
                                                                    onclick="deleted('{{ route('admin.payment.supplier-payments.destroy', $val->id) . qString() }}')"><i
                                                                    class="fa fa-close"></i> Delete</a></li>
                                                                @endif
                                                        @endcan
                                                        @can('approval supplier-payment')
                                                            @if (auth()->user()->hasRole('Super Admin') || auth()->user()->hasRole('Branch Admin') || $val->created_by != auth()->user()->id)
                                                            @if(!$val->approved_by)
                                                            <li><a onclick="activity('{{ route('admin.payment.supplier-payments.approve', $val->id) . qString() }}', 'Are you sure to approve this payment?')"><i class="fa fa-pencil"></i> Approve</a></li>
                                                            @endif
                                                            @endif
                                                        @endcan
                                                    </ul>
