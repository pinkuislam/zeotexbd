    ...
    <section class="content">
        <div class="nav-tabs-custom">
            <ul class="nav nav-tabs">
                <li class="active">
                    <a href="{{ route('admin.payment.delivery-agent-payments.index') . qString() }}">
                        <i class="fa fa-list" aria-hidden="true"></i> Delivery Agent Payment List
                    </a>
                </li>

                @can('add delivery-agent-payment')
                <li>
                    <a href="{{ route('admin.payment.delivery-agent-payments.create') . qString() }}">
                        <i class="fa fa-plus" aria-hidden="true"></i> Add Payment
                    </a>
                </li>
                <li>
                    <a href="{{ route('admin.payment.delivery-agent-payments.receive') . qString() }}">
                        <i class="fa fa-plus" aria-hidden="true"></i> Add Receive
                    </a>
                </li>
                <li>
                    <a href="{{ route('admin.payment.delivery-agent-payments.adjustment') . qString() }}">
            ...
            <div class="tab-content">
                <div class="tab-pane active">
                    <form method="GET" action="{{ route('admin.payment.delivery-agent-payments.index') }}"
                        class="form-inline">
                        <div class="box-header text-right">
                            <div class="row">
                                <div class="form-group">
                                    <select class="form-control" name="supplier">
                                        <option value="">Any Delovery Agent</option>
                                        @foreach ($deliveryagents as $item)
                                            <option value="{{ $item->id }}"
                                                {{ Request::get('supplier') == $item->id ? 'selected' : '' }}>
                                                {{ $item->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <a class="btn btn-custom btn-flat"
                                        href="{{ route('admin.payment.delivery-agent-payments.index') }}">X</a>
                                </div>
                            </div>
                        </div>
                    </form>
                                ...
                                <tbody>
                                    @foreach ($records as $val)
                                        <tr>
                                            ...
                                            <td>{{ $val->type }}</td>
                                            <td>{{ $val->deliveryAgent != null ? $val->deliveryAgent->name : '' }}</td>
                                            ...
                                            <td>
                                                <div class="dropdown">
                                                    ...
                                                    <ul class="dropdown-menu dropdown-menu-right">
                                                        @can('show delivery-agent-payment')
                                                            <li><a
                                                                    href="{{ route('admin.payment.delivery-agent-payments.show', $val->id) . qString() }}"><i
                                                                        class="fa fa-eye"></i> Show</a></li>
                                                        @endcan

                                                        @can('edit delivery-agent-payment')
                                                            @if (!$val->approved_by && auth()->user()->id == $val->created_by)   
                                                                <li><a
                                                                        href="{{ route('admin.payment.delivery-agent-payments.edit', $val->id) . qString() }}"><i
                                                                            class="fa fa-edit"></i> Edit</a></li>
                                                            @endif
                                                            @endcan
                                                            
                                                            @can('delete delivery-agent-payment')
                                                                @if (!$val->approved_by && auth()->user()->id == $val->created_by)   
                                                                <li><a
                                                                    onclick="deleted('{{ route('admin.payment.delivery-agent-payments.destroy', $val->id) . qString() }}')"><i
                                                                    class="fa fa-close"></i> Delete</a></li>
                                                                @endif
                                                        @endcan
                                                        @can('approval delivery-agent-payment')
                                                            @if (auth()->user()->hasRole('Super Admin') || auth()->user()->hasRole('Branch Admin') || $val->created_by != auth()->user()->id)
                                                            @if(!$val->approved_by)
                                                            <li><a onclick="activity('{{ route('admin.payment.delivery-agent-payments.approve', $val->id) . qString() }}', 'Are you sure to approve this payment?')"><i class="fa fa-pencil"></i> Approve</a></li>
                                                            @endif
                                                            @endif
                                                        @endcan
                                                    </ul>
