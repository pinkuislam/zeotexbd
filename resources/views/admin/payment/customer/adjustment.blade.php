        ...
        <div class="nav-tabs-custom">
            ...
            <div class="tab-content">
                <div class="tab-pane active">
                    <div class="box-body">
                        <form method="POST"
                            action="{{ isset($data) ? route('admin.payment.customer-payments.update', $data->id) : route('admin.payment.customer-payments.store') }}{{ qString() }}"
                            ...
                            <div class="row">
                                <div class="col-sm-12">
                                    ...
                                    <div class="form-group{{ $errors->has('customer_id') ? ' has-error' : '' }}">
                                        ...
                                        <div class="col-sm-9">
                                            <select name="customer_id" class="form-control select2" id="adjust_customer_id" required onchange="getAdjustDue()">
                                                <option value="">Select Customer</option>
                                                @php($customer_id = old('customer_id', isset($data) ? $data->customer_id : ''))
                                                @foreach ($customers as $cus)
                                                    <option value="{{ $cus->id }}"
                                                        {{ $customer_id == $cus->id ? 'selected' : '' }}>
                                                        {{ $cus->name . '-' . $cus->mobile . '-' . $cus->address . '-' . $cus->contact_name . '-' . $cus->type}}
                                                    </option>
                                                @endforeach
                                            </select>
    ...
    <script>
        function getAdjustDue() {
            ...
            $.ajax({
                ...
                url: "{{ route('admin.user.customers.due') }}",
                beforeSend: () => {
                    $('#adjust_due').val('');
                },
                success: (res) => {
