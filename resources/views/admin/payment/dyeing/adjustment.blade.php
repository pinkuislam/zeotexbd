        ...
        <div class="nav-tabs-custom">
            ...
            <div class="tab-content">
                <div class="tab-pane active">
                    <div class="box-body">
                        <form method="POST"
                            action="{{ isset($data) ? route('admin.payment.dyeing-payments.update', $data->id) : route('admin.payment.dyeing-payments.store') }}{{ qString() }}"
                            ...
                            <div class="row">
                                <div class="col-sm-12">
                                    ...
                                    <div class="form-group{{ $errors->has('dyeing_agent_id') ? ' has-error' : '' }}">
                                        <label class="control-label col-sm-3 required">Delivery Agent:</label>
                                        <div class="col-sm-9">
                                            <select name="dyeing_agent_id" class="form-control select2" id="adjust_dyeing_agent_id" required onchange="getAdjustDue()">
                                                <option value="">Select Delivery Agent</option>
                                                @php($dyeing_agent_id = old('dyeing_agent_id', isset($data) ? $data->dyeing_agent_id : ''))
                                                @foreach ($dyeingagents as $cat)
                                                    <option value="{{ $cat->id }}"
                                                        {{ $dyeing_agent_id == $cat->id ? 'selected' : '' }}>
                                                        {{ $cat->name }}</option>
                                                @endforeach
                                            </select>

                                            @if ($errors->has('dyeing_agent_id'))
                                                <span class="help-block">{{ $errors->first('dyeing_agent_id') }}</span>
                                            @endif
                                        </div>
                                    </div>
...
@push('scripts')
    <script>
        function getAdjustDue(){
            let id = $('#adjust_dyeing_agent_id').val();
            if(id < 1){
                return;
            }
            ...
            $.ajax({
                ...
                url: "{{ route('admin.user.dyeing-agent.due') }}",
                beforeSend: () => {
                    $('#adjust_due').val('');
                },
                success: (res) => {
