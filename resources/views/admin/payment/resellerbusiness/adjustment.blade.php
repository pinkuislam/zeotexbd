        ...
        <div class="nav-tabs-custom">
            ...
            <div class="tab-content">
                <div class="tab-pane active">
                    <div class="box-body">
                        <form method="POST"
                            action="{{ isset($data) ? route('admin.payment.reseller-business-payments.update', $data->id) : route('admin.payment.reseller-business-payments.store') }}{{ qString() }}"
                            ...
                            <div class="row">
                                <div class="col-sm-12">
                                    ...
                                    <div class="form-group{{ $errors->has('reseller_business_id') ? ' has-error' : '' }}">
                                        <label class="control-label col-sm-3 required">Reseller Business:</label>
                                        <div class="col-sm-9">
                                            <select name="reseller_business_id" id="reseller_business_id" class="form-control select2"
                                                required onchange="getAdjustDue()">
                                                ...
                                                @foreach ($reseller_business as $resbus)
                                                    <option value="{{ $resbus->id }}"
                                                        {{ $reseller_business_id == $resbus->id ? 'selected' : '' }}>
                                                        {{ $resbus->name . '-' . $resbus->mobile . '-' . $resbus->address}}</option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="form-group{{ $errors->has('total_amount') ? ' has-error' : '' }}">
                                        ...
                                        <div class="col-sm-9">
                                            <input type="number" step="any" min="0" class="form-control"
                                                name="total_amount"
                                                value="{{ old('total_amount', isset($data) ? $data->amount : '') }}"
...
@push('scripts')
    <script>
        function getAdjustDue() {
            let id = $('#reseller_business_id').val();
            if(id < 1){
                return;
            }
            ...
            $.ajax({
                ...
                url: "{{ route('admin.user.reseller_business.due') }}",
                beforeSend: () => {
                    $('#adjust_due').val('');
                },
                success: (res) => {
