        ...
        <div class="nav-tabs-custom">
            ...
            <div class="tab-content">
                <div class="tab-pane active">
                    <form method="GET" action="{{ route('admin.payment.reseller-business-payments.index') }}"
                        class="form-inline">
                        <div class="box-header text-right">
                            <div class="row">
                                <div class="form-group">
                                    <select class="form-control select2" name="reseller_business">
                                        <option value="">Any Reseller Business</option>
                                        @foreach ($reseller_business as $item)
                                            <option value="{{ $item->id }}"
                                                {{ Request::get('reseller_business') == $item->id ? 'selected' : '' }}>
                                                {{ $item->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <a class="btn btn-custom btn-flat"
                                        href="{{ route('admin.payment.reseller-business-payments.index') }}">X</a>
                                </div>
                            </div>
                        </div>
                    </form>
                                ...
                                <tbody>
                                    @foreach ($records as $val)
                                        <tr>
                                            ...
                                            <td>{{ $val->type }}</td>
                                            <td>{{ $val->amount }}</td>
                                            <td>{{ $val->resellerBusiness != null ? $val->resellerBusiness->name : '' }}</td>
                                            <td>{{ $val->note }}</td>
                                            ...
                                            <td>
                                                <div class="dropdown">
                                                    ...
                                                    <ul class="dropdown-menu dropdown-menu-right">
                                                        @can('show reseller-business-payments')
                                                            <li><a
                                                                    href="{{ route('admin.payment.reseller-business-payments.show', $val->id) . qString() }}"><i
                                                                        class="fa fa-eye"></i> Show</a></li>
                                                        @endcan
                                                        
                                                        @can('edit reseller-business-payments')
                                                            @if (!$val->approved_by && auth()->user()->id == $val->created_by)   
                                                            <li><a
                                                                    href="{{ route('admin.payment.reseller-business-payments.edit', $val->id) . qString() }}"><i
                                                                        class="fa fa-edit"></i> Edit</a></li>
                                                            @endif
                                                                @endcan
                                                                @can('delete reseller-business-payments')
                                                                @if (!$val->approved_by && auth()->user()->id == $val->created_by)   
                                                                    <li><a
                                                                            onclick="deleted('{{ route('admin.payment.reseller-business-payments.destroy', $val->id) . qString() }}')"><i
                                                                                class="fa fa-close"></i> Delete</a></li>
                                                                @endif
                                                            @endcan
                                                                            
                                                            @can('approval reseller-business-payments')
                                                            @if (auth()->user()->hasRole('Super Admin') || auth()->user()->hasRole('Branch Admin') || $val->created_by != auth()->user()->id)
                                                            @if (!$val->approved_by)
                                                                <li><a
                                                                        onclick="activity('{{ route('admin.payment.reseller-business-payments.approve', $val->id) . qString() }}', 'Are you sure to approve this payment?')"><i
