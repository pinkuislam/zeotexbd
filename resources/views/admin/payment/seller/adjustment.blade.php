        ...
        <div class="nav-tabs-custom">
            ...
            <div class="tab-content">
                <div class="tab-pane active">
                    <div class="box-body">
                        <form method="POST"
                            action="{{ isset($data) ? route('admin.payment.seller-payments.update', $data->id) : route('admin.payment.seller-payments.store') }}{{ qString() }}"
                            ...
                            <div class="row">
                                <div class="col-sm-12">
                                    ...
                                    <div class="form-group{{ $errors->has('seller_id') ? ' has-error' : '' }}">
                                        <label class="control-label col-sm-3 required">seller:</label>
                                        <div class="col-sm-9">
                                            <select name="seller_id" class="form-control select2" id="adjust_seller_id" required onchange="getAdjustDue()">
                                                <option value="">Select seller</option>
                                                @php($seller_id = old('seller_id', isset($data) ? $data->seller_id : ''))
                                                @foreach ($sellers as $cat)
                                                    <option value="{{ $cat->id }}"
                                                        {{ $seller_id == $cat->id ? 'selected' : '' }}>
                                                        {{ $cat->name }}</option>
                                                @endforeach
                                            </select>
    ...
    <script>
        function getAdjustDue(){
            ...
            $.ajax({
                ...
                url: "{{ route('admin.user.sellers.due') }}",
                beforeSend: () => {
                    $('#adjust_due').val('');
                },
                success: (res) => {
