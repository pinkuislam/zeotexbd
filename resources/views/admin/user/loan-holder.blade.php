        ...
        <div class="nav-tabs-custom">
            <ul class="nav nav-tabs">
                <li {{ isset($list) ? 'class=active' : '' }}>
                    <a href="{{ route('admin.user.loan-holder.index') . qString() }}">
                        <i class="fa fa-list" aria-hidden="true"></i> Loan Holder List
                    </a>
                </li>
                @can('add loan-holder')
                    <li {{ isset($create) ? 'class=active' : '' }}>
                        <a href="{{ route('admin.user.loan-holder.create') . qString() }}">
                            <i class="fa fa-plus" aria-hidden="true"></i> Add Loan Holder
                        </a>
                    </li>
                @endcan

                @can('edit loan-holder')
                    @if (isset($edit))
                        <li class="active">
                            <a href="#">
                                <i class="fa fa-edit" aria-hidden="true"></i> Edit Loan Holder
                            </a>
                        </li>
                    @endif
                @endcan

                @can('show loan-holder')
                    @if (isset($show))
                        <li class="active">
                            <a href="#">
                                <i class="fa fa-list-alt" aria-hidden="true"></i> Loan Holder Details
                            </a>
                        </li>
                    @endif
                @endcan
            ...
            <div class="tab-content">
                ...
                @elseif(isset($edit) || isset($create))
                    <div class="tab-pane active">
                        <div class="box-body">
                            <form method="POST"
                                action="{{ isset($edit) ? route('admin.user.loan-holder.update', $edit) : route('admin.user.loan-holder.store') }}{{ qString() }}"
                ...
                @elseif (isset($list))
                    <div class="tab-pane active">
                        <form method="GET" action="{{ route('admin.user.loan-holder.index') }}" class="form-inline">
                            <div class="box-header text-right">
                                <div class="row">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <a class="btn btn-warning btn-flat" href="{{ route('admin.user.loan-holder.index') }}">X</a>
                                    </div>
                                </div>
                            </div>
                        </form>
                                ...
                                <tbody>
                                    @foreach ($loanHolders as $val)
                                        <tr>
                                            ...
                                            <td>
                                                @canany(['show loan-holder', 'edit loan-holder', 'delete loan-holder'])
                                                    <div class="dropdown">
                                                        ...
                                                        <ul class="dropdown-menu dropdown-menu-right">
                                                            @can('show loan-holder')
                                                                <li><a href="{{ route('admin.user.loan-holder.show', $val->id) . qString() }}"><i
                                                                            class="fa fa-eye"></i> Show</a></li>
                                                            @endcan

                                                            @can('edit loan-holder')
                                                                <li><a href="{{ route('admin.user.loan-holder.edit', $val->id) . qString() }}"><i
                                                                            class="fa fa-pencil"></i> Edit</a></li>
                                                            @endcan
                                                            @can('delete loan-holder')
                                                                <li><a
                                                                        onclick="deleted('{{ route('admin.user.loan-holder.destroy', $val->id) . qString() }}')"><i
                        ...
                        <div class="row">
                            <div class="col-sm-4 pagi-msg">{!! pagiMsg($loanHolders) !!}</div>

                            <div class="col-sm-4 text-center">
                                {{ $loanHolders->appends(Request::except('page'))->links() }}
                            </div>
