    ...
    <section class="content">
        <div class="nav-tabs-custom">
            <ul class="nav nav-tabs">
                ...
                <li>
                    <a href="{{ route('admin.report.finished-product-ledger') }}">
            ...
            <div class="tab-content">
                <div class="tab-pane active">
                    <form method="GET" action="{{ route('admin.report.finished-product-stock', $type) }}" class="form-inline">
                            ...
                            <div class="row">
                                ...
                                <div class="form-group">
                                    ...
                                    <a class="btn btn-warning btn-flat"
                                        href="{{ route('admin.report.finished-product-stock', $type) }}">X</a>
                                </div>
                            </div>
                        </div>
                    </form>
                        ...
                        <table class="table table-bordered table-hover">
                            <thead>
                                <tr>
                                    ...
                                    <th style="text-align: right;">Purchase Return</th>
                            ...
                            <tbody>
                                @foreach ($reports as $key=>$val)
                                    <tr>
                                        <td>{{ $key+1}}</td>
                                        <td><a href="{{ route('admin.report.finished-product-ledger') . '?product_id=' . $val->id }}">{{ $val->product_code }}</a></td>
                            ...
                            <tfoot>
                                <tr>
                                    <th style="text-align:right" colspan="4">Total Qty</th>
                                    <th style="text-align:right">
                                        {{ $reports->sum('production_quantity') }}</th>
                                    <th style="text-align:right">
                                        {{ $reports->sum('purchase_quantity') }}</th>
                                    <th style="text-align:right">
                                        {{ $reports->sum('sale_return_quantity') }}</th>
                                    <th style="text-align:right">
                                        {{ $reports->sum('sale_quantity') }}</th>
                                    <th style="text-align:right">
                                        {{ $reports->sum('damage_quantity') }}</th>
                                    <th style="text-align:right">
                                        {{ $reports->sum('purchase_return_quantity') }}</th>
                                    <th style="text-align:right">
                                        {{ $reports->sum('stock_quantity') }}</th>
                                    <th style="text-align:right">
                                        {{  number_format( $reports->sum('stock_amount') , 2) }}</th>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
